...
from core.cutils import addlines, addstr, move, footer, getch, reset_footer, reset_line


def setup_mysql(stdscr, proceed_to_username=True):
    ...
    movements = [[7, 10], [6, 10], [5, 6], [4, 6]]
    current_pos = 0
    pos_to_data = {4: "host", 5: "port", 6: "username", 7: "password"}

    while True:
        addstr(stdscr, 0, 0, "seal", curses.color_pair(4) | curses.A_BOLD)
        addstr(stdscr, 0, 4, f" — MySQL Setup", curses.A_BOLD)
        addstr(stdscr, 2, 0, "Set up MySQL connection", curses.A_BOLD)
        footer(
            stdscr,
            "Use [▲] and [▼] arrow keys to navigate, [ESC] to go back, and [Enter] to confirm.",
        )
        addstr(stdscr, 4, 0, f"Host: {data['host']}", colors[0])
        addstr(stdscr, 5, 0, f"Port: {data['port']}", colors[1])
        addstr(stdscr, 6, 0, f"Username: {data['username']}", colors[2])
        addstr(stdscr, 7, 0, f"Password: {'*' * len(data['password'])}", curses.A_BOLD)
        ...
        if ch == curses.KEY_UP and current_pos < 3:
            ...
            if not (1 <= int(data["port"]) <= 65535):
                addstr(
                    stdscr, 9, 0, "[!] Invalid port", curses.color_pair(3), reset=True
                )
            ...
            def handle_key_press(allowable_characters, max_length):
                if chr(ch) in allowable_characters and len(data[editing]) < max_length:
                    data[editing] += chr(ch)
            ...
            if editing == "password":
                handle_key_press("".join(chr(i) for i in range(32, 127)), 64)
...
are correct.""",
        ...
        if ch != 27:  #* ESC key
            setup_mysql(stdscr, proceed_to_username=False)
        else:
            stdscr.clear()
            break
