...
def choose_master_password(stdscr, username: str):
    ...
    while True:
        ...
        addstr(stdscr, 0, 4, f" — Set Master Password", curses.A_BOLD)
        addstr(stdscr, 2, 0, "Create master password:", curses.A_BOLD)
        ...
        if ch in (curses.KEY_ENTER, 10, 13):
            if all(requirements.values()):
               #* Confirm password segment
                ...
                addstr(stdscr, 9, 0, "Password hidden", curses.color_pair(6), reset=True)
                addstr(stdscr, 10, 0, f"Password: {'*' * len(master_password)} ", reset=True)
                ...
                while True:
                    ...
                    move(
                        stdscr, 12, 18 + len(confirm_password)
